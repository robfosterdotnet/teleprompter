...
- **2.2 Stylelint or tailwind config** — Configure Tailwind `tailwind.config.ts` and optional stylelint rules for custom CSS. _Result:_ Consistent utility usage logged in docs. _Status:_ Not Started.
- **2.3 TypeScript strictness** — Enable `strict`, `noImplicitAny`, `strictNullChecks`, etc., and fix resulting errors. _Result:_ Compiler enforces strong typing. _Status:_ Not Started.
- **2.4 Husky hooks** — Add pre-commit hook running lint + tests (or lint-staged). _Result:_ Failing lint/test blocks commit. _Status:_ Not Started.
- **2.5 CI workflow** — Create GitHub Actions workflow that installs deps, lints, tests, and uploads coverage artifact. _Result:_ Green check on PRs when lint/tests pass. _Status:_ Not Started.
- **2.6 Editor settings** — Publish `.editorconfig` and VS Code recommendations to keep indentation/formatting consistent. _Result:_ IDEs auto-format per team standards. _Status:_ Not Started.
